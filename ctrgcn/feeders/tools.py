...
def resize_window(data, window,cropped_length):
    C, T, V, M = data.shape
...
def resize_window_torch(data, window,cropped_length):
    data = data.numpy()
    data = resize_window(data, window,cropped_length)
    data = torch.tensor(data, dtype=torch.float)
    return data
...
def get_begin_end(data):
    ...
    for i in range(T):
        if np.sum(data[:,i,:,:].sum(0).sum(-1).sum(-1) != 0) != 0:
            begin = i
            break
...
def valid_crop_resize(data_numpy,p_interval,window):
    # input: C,T,V,M
    C, T, V, M = data_numpy.shape
    if np.sum(data_numpy[:, :, :, 1].sum(0).sum(-1) != 0) == 0:
        data_numpy[:, :, :, 1] = data_numpy[:, :, :, 0]
    begin, end = get_begin_end(data_numpy)
    valid_size = end - begin + 1
    # crop
    if len(p_interval) == 1:
        p = p_interval[0]
...
def valid_crop_resize_torch(data,p_interval,window):
    data = data.numpy()
    data = valid_crop_resize(data, p_interval, window)
    data = torch.tensor(data, dtype=torch.float)
    return data
