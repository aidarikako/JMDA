...
def objective(weights):
    ...
    if len(weights) == 3:
        for i in tqdm(range(len(label))):
            ...
            r = r11 * weights[0] + r22 * weights[1] + r55 * weights[2]
            r = np.argmax(r)
...
if __name__ == "__main__":
    ...
    dataset_path = "/media/xlh/ed9a7cd9-c5e6-4056-b172-ad8756a6ba8a/dataset/NTU_skeleton/CTR-GCN/"
    ...
    if 'UCLA' in arg.dataset:
        ...
        if 'xsub' in arg.dataset:
            npz_data = np.load(dataset_path + 'ntu120/' + 'NTU120_CSub.npz')
            label = np.where(npz_data['y_test'] > 0)[1]
        elif 'xset' in arg.dataset:
            npz_data = np.load(dataset_path + 'ntu120/' + 'NTU120_CSet.npz')
            label = np.where(npz_data['y_test'] > 0)[1]
    elif 'ntu' in arg.dataset:
        if 'xsub' in arg.dataset:
            npz_data = np.load(dataset_path + 'ntu/' + 'NTU60_CS.npz')
            label = np.where(npz_data['y_test'] > 0)[1]
        elif 'xview' in arg.dataset:
            npz_data = np.load(dataset_path + 'ntu/' + 'NTU60_CV.npz')
            label = np.where(npz_data['y_test'] > 0)[1]
    ...
    if arg.joint_motion_dir is None:
        space = [(0.2, 1.2) for i in range(3)]
    elif arg.joint_motion_dir is not None and arg.joint_motion_k2_dir is None:
